...
function allTagsLoad() {
    $.ajax({
        type: 'GET',
        url: '/fe_blog/SelectAllTagController',
        data: {},
        dataType: "json",
        ...
        $.ajax({
            ...
            success: function (res2) {
                ...
                for (let j = 0; j < res2.length; j++) {
                    let element2 = `
                        <li class="tags_article_item" id="${res2[j]['blogid']}">
                            <a href="article.html?blogId=${res2[j]['blogid']}"><span>${res2[j]['title']}</span></a>
                        </li>
                        `
                    list.push(element2);
                }
